...
export class Syncer {
  ...
  async syncSessionFile(filePath: string) {
    try {
      ...
      const firstMsg = data.messages.find(m => m.type !== 'info');
      const summary = firstMsg ? extractMessageSummary(firstMsg.content) : undefined;
          ...
          if (msg.toolCalls && Array.isArray(msg.toolCalls)) {
            for (const tc of msg.toolCalls) {
              await tx.insert(toolCalls).values({
                ...
                target: toolCalls.id,
                set: { result: JSON.stringify(tc.result), status: tc.status }
              });
            }
          }
        }
    ...
    try {
      ...
      await db.transaction(async (tx) => {
        ...
        if (session?.projectId) {
          await tx.update(projects).set({ progress }).where(eq(projects.id, session.projectId));
